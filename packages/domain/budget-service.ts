...
export const BUDGET_CALCULATION_VERSION = 1;

/** Minimal transaction shape the service needs; avoids coupling to full PostedTransaction */
export interface BudgetTransaction {
    id: string;
    category: string | null;
    amountCents: number; // positive = debit/expense; negative = credit/refund
...
export interface CalculateBudgetInput {
    ...
    period: BudgetPeriod;
    budgets: Budget[];
    transactions: BudgetTransaction[]; // pre-filtered to the period by the caller
    asOf: Date;                        // "now"; used for projectedMonthEnd
}
 ...
 */
function daysElapsed(asOf: Date, year: number, month: number): number {
 ...
 */
function projectMonthEnd(actualCents: number, elapsed: number, totalDays: number): Money {
    if (elapsed === 0 || elapsed >= totalDays) {
        return actualCents as Money;
    }
    return Math.ceil((actualCents * totalDays) / elapsed) as Money;
}

function computeStatus(hasBudget: boolean, plannedCents: number, actualCents: number): BudgetStatus {
    if (!hasBudget) return BudgetStatus.UNBUDGETED;
    if (actualCents === 0) return BudgetStatus.NO_SPENDING;
    if (actualCents > plannedCents) return BudgetStatus.OVER_BUDGET;
...
function calculateCategoryResult(
    ...
    const actualCents = relevant.reduce((sum, t) => sum + t.amountCents, 0) as Money;
    const remainingCents = (plannedCents - actualCents) as Money;
    const varianceCents = (actualCents - plannedCents) as Money;
    ...
    return {
        category,
        period,
        plannedCents: plannedCents as Money,
        actualCents,
        remainingCents,
        varianceCents,
        variancePercent,
...
export class BudgetService {
    ...
    calculateResults(input: CalculateBudgetInput): BudgetResultSet {
        ...
        for (const category of allCategories) {
            ...
            results.push(calculateCategoryResult(
                category,
                period,
        ...
        const totalPlannedCents = results
            ...
            .reduce((s, r) => s + r.plannedCents, 0) as Money;

        const totalActualCents = results
            .reduce((s, r) => s + r.actualCents, 0) as Money;

        const totalRemainingCents = (totalPlannedCents - totalActualCents) as Money;
        const totalVarianceCents = (totalActualCents - totalPlannedCents) as Money;
