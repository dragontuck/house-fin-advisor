...
exports.DebtIntelligenceService = exports.DEBT_INTELLIGENCE_VERSION = void 0;
exports.createDebtIntelligenceService = createDebtIntelligenceService;
const contracts_1 = require("@house-fin/contracts");
exports.DEBT_INTELLIGENCE_VERSION = 1;
// ── Category mapping ──────────────────────────────────────────────────────────
function categoryFor(type) {
    switch (type) {
        case contracts_1.AccountType.CREDIT_CARD: return contracts_1.DebtCategory.REVOLVING;
        case contracts_1.AccountType.LOAN: return contracts_1.DebtCategory.INSTALLMENT;
        case contracts_1.AccountType.MORTGAGE: return contracts_1.DebtCategory.MORTGAGE;
        default: return contracts_1.DebtCategory.UNKNOWN;
    }
}
const DEBT_ACCOUNT_TYPES = new Set([
...
function buildDetail(account) {
    ...
    const currentBalanceCents = Math.abs(account.currentBalance);
    let utilizationRatio = null;
    if (category === contracts_1.DebtCategory.REVOLVING && creditLimitCents !== null && creditLimitCents > 0) {
 ...
 */
function weightedAvgBps(details) {
    const active = details.filter(d => d.currentBalanceCents > 0);
    if (active.length === 0)
        return null;
    if (active.some(d => d.interestRateBps === null))
        return null;
    const totalBalance = active.reduce((s, d) => s + d.currentBalanceCents, 0);
    if (totalBalance === 0)
        return null;
    const weighted = active.reduce((s, d) => s + d.interestRateBps * d.currentBalanceCents, 0);
    return Math.round(weighted / totalBalance);
}
/** Sum a payment field; null if ANY active debt account is missing the field. */
function sumPayments(details, field) {
    const active = details.filter(d => d.currentBalanceCents > 0);
    if (active.length === 0)
        return 0;
    if (active.some(d => d[field] === null))
        return null;
    return active.reduce((s, d) => s + d[field], 0);
}
// ── Status determination ──────────────────────────────────────────────────────
function determineStatus(details, dti) {
    // Start at HEALTHY and escalate.
    let status = contracts_1.DebtHealthStatus.HEALTHY;
    const escalate = (s) => {
        const order = [
            contracts_1.DebtHealthStatus.HEALTHY,
            contracts_1.DebtHealthStatus.WATCH,
            contracts_1.DebtHealthStatus.AT_RISK,
            contracts_1.DebtHealthStatus.CRITICAL,
        ];
        if (order.indexOf(s) > order.indexOf(status))
            status = s;
    };
    // DTI thresholds
    if (dti !== null) {
        if (dti > 0.43)
            escalate(contracts_1.DebtHealthStatus.CRITICAL);
        else if (dti > 0.36)
            escalate(contracts_1.DebtHealthStatus.AT_RISK);
        else if (dti > 0.28)
            escalate(contracts_1.DebtHealthStatus.WATCH);
    }
    // Per-card revolving utilisation
    for (const d of details) {
        if (d.category !== contracts_1.DebtCategory.REVOLVING)
            continue;
    ...
    const hasRevolvingDebt = details.some(d => d.category === contracts_1.DebtCategory.REVOLVING && d.revolvingBalanceCents !== null && d.revolvingBalanceCents > 0);
    if (hasRevolvingDebt)
        escalate(contracts_1.DebtHealthStatus.WATCH);
    return status;
}
// ── Observations ──────────────────────────────────────────────────────────────
function buildObservations(details, totalDebt, weightedRate, dti) {
    const obs = [];
    // No debt at all
    if (totalDebt === 0) {
    ...
    const revolving = details.filter(d => d.category === contracts_1.DebtCategory.REVOLVING);
    ...
    const otherRates = details
        .filter(d => d.category !== contracts_1.DebtCategory.REVOLVING && d.interestRateBps !== null)
...
// ── Status description ────────────────────────────────────────────────────────
function buildStatusDescription(status, totalDebt, dti, details) {
    if (totalDebt === 0)
        return "Your household carries no outstanding debt.";
    switch (status) {
        case contracts_1.DebtHealthStatus.HEALTHY:
        ...
        case contracts_1.DebtHealthStatus.WATCH: {
            const util = details.find(d => d.category === contracts_1.DebtCategory.REVOLVING && d.utilizationRatio !== null && d.utilizationRatio > 0.30);
            if (util) {
        ...
        case contracts_1.DebtHealthStatus.AT_RISK: {
            const high = details.find(d => d.category === contracts_1.DebtCategory.REVOLVING && d.utilizationRatio !== null && d.utilizationRatio > 0.50);
            if (high) {
        ...
        case contracts_1.DebtHealthStatus.CRITICAL: {
            const critical = details.find(d => d.category === contracts_1.DebtCategory.REVOLVING && d.utilizationRatio !== null && d.utilizationRatio > 0.75);
            if (critical) {
...
class DebtIntelligenceService {
    analyze(input) {
        ...
        const debtAccounts = accounts.filter(a => DEBT_ACCOUNT_TYPES.has(a.type) && a.status === "ACTIVE");
        const details = debtAccounts.map(buildDetail);
        const totalDebtCents = details.reduce((s, d) => s + d.currentBalanceCents, 0);
        const revolvingDebtCents = sumBy(details, contracts_1.DebtCategory.REVOLVING);
        const installmentDebtCents = sumBy(details, contracts_1.DebtCategory.INSTALLMENT);
        const mortgageDebtCents = sumBy(details, contracts_1.DebtCategory.MORTGAGE);
        ...
        const debtToIncomeRatio = paymentForDti !== null && monthlyIncomeCents > 0
        ...
        return {
            householdId,
            asOf,
            calculationVersion: exports.DEBT_INTELLIGENCE_VERSION,
            totalDebtCents,
            revolvingDebtCents,
...
exports.DebtIntelligenceService = DebtIntelligenceService;
function createDebtIntelligenceService() {
    return new DebtIntelligenceService();
}
//# sourceMappingURL=debt-intelligence-service.js.map