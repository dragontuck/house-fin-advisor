...
async function extractTextFromImage(buffer: Buffer, mimeType: string): Promise<string> {
    ...
    throw new Error(
        "Image OCR processing requires Tesseract.js configuration. Please run: npm install tesseract.js"
    );
}
 ...
 */
function extractImageMetadata(ocrText: string): {
    institution?: string;
    periodStart?: Date;
    periodEnd?: Date;
    accountHints: string[];
 ...
 */
function parseImageDate(dateStr: string): Date {
 ...
 */
function detectAccountTypeFromOcrText(text: string): AccountType | undefined {
 ...
 */
function extractBalanceFromOcr(text: string): { opening?: number; closing?: number } {
 ...
 */
export class ImageStatementParser implements StatementParser {
    /**
     * Check if this parser can handle image input
     */
    async canParse(input: ParserInput): Promise<ParserMatch> {
        if (input.sourceType !== DocumentSourceType.IMAGE) {
            return {
                matches: false,
                confidence: 0,
    ...
    async parse(input: ParserInput): Promise<ExtractedParsedStatement> {
        ...
        return {
            ...
            transactions,
            extractionMethods: [ExtractionMethod.OCR],
