...
describe('OnboardingProgress Model', () => {
    ...
    describe('State Transitions', () => {
        ...
        it('should track transition to COMPLETE with completion timestamp', () => {
            ...
            const progress: OnboardingProgress = {
                ...
                phases: {
                    1: { completed: true, completedAt: new Date(), data: {} },
                    2: { completed: true, completedAt: new Date(), data: {} },
                    3: { completed: true, completedAt: new Date(), data: {} },
                    4: { completed: true, completedAt: new Date(), data: {} },
                    5: { completed: true, completedAt: new Date(), data: {} },
                    6: { completed: true, completedAt: completedAt, data: {} },
                },
                startedAt: new Date('2026-09-13T09:00:00Z'),
    ...
    describe('Checkpoint Tracking', () => {
        it('should store latest checkpoint for resume functionality', () => {
            ...
            expect(progress.lastCheckpoint?.phase).toBe(1);
            expect(progress.lastCheckpoint?.sessionId).toBe('session-abc123');
        });

        it('should update checkpoint when resuming onboarding', () => {
            const oldCheckpoint = { phase: 1, completedAt: new Date(), sessionId: 'old-session' };
            const newCheckpoint = { phase: 2, completedAt: new Date(), sessionId: 'new-session' };
            ...
            expect(progress.lastCheckpoint?.sessionId).toBe('new-session');
            expect(progress.lastCheckpoint?.phase).toBe(2);
        });
    });
