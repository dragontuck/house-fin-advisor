 ...
 */
function identifyDateColumn(rows: string[][], minSampleSize = 3): number | null {
 ...
 */
function detectAmountRepresentation(rows: string[][]): {
    ...
    let signedAmountColumn: number | undefined;
    ...
    for (let colIndex = 0; colIndex < (rows[0]?.length || 0); colIndex++) {
        ...
        if (numericValues.length >= 2) {
            ...
            if (
                parsed.length === numericValues.length &&
                parsed.some((v) => v! > 0) &&
                parsed.some((v) => v! < 0)
            ) {
 ...
 */
function mapColumns(rows: string[][], headerRowIndex: number): ColumnMapping {
    ...
    const patterns: Record<string, { keywords: string[]; key: keyof ColumnMapping }> = {
        date: {
    ...
    for (let colIndex = 0; colIndex < headerRow.length; colIndex++) {
        ...
        for (const [, { keywords, key }] of Object.entries(patterns)) {
            if (keywords.some((kw) => headerCell.includes(kw))) {
                const currentValue = mapping[key] as number | null;
                if (currentValue !== null) {
                    ambiguities.push(`Multiple ${key} candidates found`);
                } else {
                    (mapping[key] as number) = colIndex;
                }
            }
        }
    }
...
function parseTransactions(
    ...
    const transactions: TransactionCandidate[] = [];
    const warnings: ParsedStatement["warnings"] = [];
    ...
    for (let rowIndex = 0; rowIndex < dataRows.length; rowIndex++) {
        ...
        if (mapping.amountColumnIndex !== null && mapping.amountColumnIndex < row.length) {
            ...
            mapping.debitColumnIndex !== null &&
            mapping.creditColumnIndex !== null
        ) {
            // Extract from debit/credit columns
            const debitStr =
                mapping.debitColumnIndex < row.length ? row[mapping.debitColumnIndex] : "";
            const creditStr =
                mapping.creditColumnIndex < row.length ? row[mapping.creditColumnIndex] : "";
...
export class CsvStatementParser implements StatementParser {
    async canParse(input: ParserInput): Promise<ParserMatch> {
        if (input.sourceType !== DocumentSourceType.CSV) {
            return {
                matches: false,
                confidence: 0,
        ...
        try {
            ...
            if (
                ...
                return {
                    ...
                    reason:
                        "Could not identify required columns (date, description, amount or debit/credit)",
                };
            }
    ...
    async parse(input: ParserInput): Promise<ParsedStatement> {
        try {
            const csvContent = typeof input.fileContent === 'string'
                ? input.fileContent
                : input.fileContent.toString('utf-8');
            ...
            if (mapping.confidence < 0.6) {
                const missingFields = [];
                if (mapping.dateColumnIndex === null) missingFields.push("date");
                if (mapping.descriptionColumnIndex === null) missingFields.push("description");
                if (mapping.amountColumnIndex === null &&
                    (mapping.debitColumnIndex === null || mapping.creditColumnIndex === null)) {
                    missingFields.push("amount");
                }
                throw new Error(
                    `Unable to detect statement structure with confidence (${mapping.confidence}). Missing/ambiguous required fields: ${missingFields.join(", ")}. Issues: ${mapping.ambiguities.join("; ")}`
                );
            }

            if (mapping.dateColumnIndex === null) {
                throw new Error(
                    "Could not identify date column. Rejected ambiguous mapping rather than guessing."
                );
            }

            if (mapping.descriptionColumnIndex === null) {
                throw new Error(
                    "Could not identify description column. Rejected ambiguous mapping rather than guessing."
                );
            }
            ...
            if (
                ...
                throw new Error(
                    "Could not identify amount columns (need either 'Amount' or both 'Debit' and 'Credit' columns). Rejected ambiguous mapping rather than guessing."
                );
            }
            ...
            if (transactions.length === 0) {
                throw new Error(
                    "No valid transactions could be parsed from CSV. Check file format and column structure."
                );
            }
            ...
            throw new Error(
                `Failed to parse CSV statement: ${error instanceof Error ? error.message : "Unknown error"}`
            );
        }
    }
}
